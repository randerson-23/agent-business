...
35. **Sponsor click reporting and a monthly member report.** This is the
    direct consequence of item 29 shipping: memberships renew annually, and
    the renewal conversation needs evidence. The industry finding is blunt —
    absent structured reporting, pricing power and renewal rates both fall.
    GoatCounter (item 23) can already count outbound clicks, so the work is
    to instrument member and sponsor links as events and have the build
    generate a one-page monthly report per member: impressions, clicks,
    which guide or region drove them. Generated by the pipeline, so it costs
    the owner nothing per month — and it is the difference between "renew?"
    being a judgment call and being a number. **Highest-value open item on
    the board.**
